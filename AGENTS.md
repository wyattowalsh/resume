...
- Keep web-only skill popover display data in `assets/data/skill-details.json` using the `{ name, desc, icon, links }` shape; descriptions should stay compact and describe the skill itself, links should be curated canonical HTTPS references such as homepages, docs, source repos, package listings, specs, or app-store listings as appropriate, and this data must not appear in generated PDF/DOCX artifacts.
