...
const siteVariantPath   = path.join(projectRoot, "assets/data/variants/site.json");
const skillDetailsPath  = path.join(projectRoot, "assets/data/skill-details.json");
...
const tempIconOutputDir = path.join(projectRoot, "public", `.skill-icons-${process.pid}`);
...
const iconSourceSchema = z.enum(["official", "favicon", "legacy", "web-sourced", "custom", "devicon"]);
...
type DownloadedIcon = Omit<IconSource, "category" | "iconPath" | "sha256" | "sourceUrl"> & {
  bytes: Buffer;
  extension: string;
  sourceUrl: string;
};
...
const curatedIconCandidates: Record<string, CuratedIconCandidate[]> = {
  ChatGPT: [customIcon("chatgpt", "https://upload.wikimedia.org/wikipedia/commons/0/04/ChatGPT_logo.svg")],
  "GitHub Copilot Web": [deviconIcon("github"), openMojiIcon("robot", "1F916")],
  "GitHub Copilot CLI": [deviconIcon("github"), openMojiIcon("terminal", "1F5A5")],
  "Gemini CLI": [openMojiIcon("gem", "1F48E"), openMojiIcon("terminal", "1F5A5")],
  Antigravity: [openMojiIcon("rocket", "1F680")],
  "Perplexity Desktop": [openMojiIcon("compass", "1F9ED")],
  ...
  BigQuery: [openMojiIcon("magnifying-glass", "1F50D")],
  Redshift: [openMojiIcon("warehouse", "1F3ED")],
  "AWS Athena": [openMojiIcon("magnifying-glass", "1F50D")],
  "AWS Glue": [openMojiIcon("link", "1F517")],
  Prefect: [openMojiIcon("route", "1F5FA")],
  Parquet: [openMojiIcon("package", "1F4E6")],
  ...
  Matplotlib: [openMojiIcon("chart-increasing", "1F4C8")],
  ...
  "OpenID Connect": [openMojiIcon("identification-card", "1FAAA")],
  ...
  MobX: [openMojiIcon("satellite-antenna", "1F4E1")],
  MDX: [openMojiIcon("memo", "1F4DD")],
  Electron: [deviconIcon("electron")],
  CodeMirror: [openMojiIcon("memo", "1F4DD")],
  ...
  CloudFormation: [openMojiIcon("cloud", "2601")],
  Helm: [deviconIcon("helm")],
  "Argo CD": [deviconIcon("argocd")],
  Jenkins: [deviconIcon("jenkins")],
  CircleCI: [deviconIcon("circleci", "plain")],
  "AWS Lambda": [openMojiIcon("high-voltage", "26A1")],
  "Google Cloud Functions": [openMojiIcon("cloud", "2601")],
  ECS: [openMojiIcon("package", "1F4E6")],
  EKS: [openMojiIcon("package", "1F4E6")],
  ...
  Puppeteer: [openMojiIcon("performing-arts", "1F3AD")],
  Playwright: [openMojiIcon("performing-arts", "1F3AD")],
  ...
  Formatting: [openMojiIcon("straight-ruler", "1F4CF")],
  "Domain-Driven Design": [openMojiIcon("compass", "1F9ED")],
  "Security-Aware Development": [openMojiIcon("shield", "1F6E1")],
...
function customIcon(iconName: string, url: string): CuratedIconCandidate {
  return { iconName, source: "custom", url };
}

function deviconIcon(iconName: string, variant = "original"): CuratedIconCandidate {
  return {
    iconName,
    source: "devicon",
    sourceNote: "Devicon ecosystem icon; treated as an official source-traced ecosystem asset.",
    url: `https://raw.githubusercontent.com/devicons/devicon/master/icons/${iconName}/${iconName}-${variant}.svg`,
  };
}

function openMojiIcon(iconName: string, codepoint: string): CuratedIconCandidate {
  return {
    iconName,
    licenseNote: "Curated OpenMoji SVG under CC BY-SA 4.0; used only as an explicit unique concept icon.",
    source: "custom",
    sourceNote: "Explicit curated concept icon, not a generic category fallback.",
...
function sourceCounts(manifest: IconManifest): Record<IconSource["source"], number> {
  const counts = Object.fromEntries(iconSourceSchema.options.map((source) => [source, 0])) as Record<
    IconSource["source"],
    number
  >;
...
function isGenericOfficialIconHost(officialUrl: string): boolean {
  ...
  return hostname === "github.com" || hostname === "arxiv.org" || hostname.endsWith(".readthedocs.io");
}

function iconExtension(url: string, contentType: string, bytes: Buffer): string | undefined {
  ...
  if (normalizedContentType === "image/svg+xml" || pathname.endsWith(".svg")) {
    return prefix.startsWith("<svg") || prefix.startsWith("<?xml") ? "svg" : undefined;
  }
  ...
  if (normalizedContentType === "image/jpeg" || pathname.endsWith(".jpg") || pathname.endsWith(".jpeg")) {
    return "jpg";
  }
...
function assertSafeSvg(bytes: Buffer, sourceUrl: string): void {
  ...
  const unsafePatterns: Array<[RegExp, string]> = [
    ...
    [/\b(?:href|src|xlink:href)\s*=\s*(["'])\s*(?:https?:|\/\/|javascript:|data:)/i, "unsafe href/src"],
    [/<!\s*(?:doctype|entity|notation|\[CDATA\[)/i, "unsafe XML construct"],
    [/<\?(?!xml(?:\s|\?>))/i, "unsafe processing instruction"],
    [/@import\b|url\(\s*(["'])?\s*(?:https?:|\/\/|javascript:|data:)/i, "unsafe CSS reference"],
  ];
...
async function fetchIcon(url: string): Promise<{ bytes: Buffer; extension: string } | undefined> {
  const response = await fetchWithTimeout(url, {
  ...
  const extension = iconExtension(url, response.headers.get("content-type") ?? "", bytes);
...
function isFaviconUrl(url: string): boolean {
  ...
  return pathname.includes("favicon") || pathname.includes("apple-touch-icon") || pathname.endsWith("/icon.svg");
}
...
function dedupeIconCandidates(candidates: IconUrlCandidate[]): IconUrlCandidate[] {
...
async function* resolveOfficialIconOverrides(skillName: string): AsyncGenerator<DownloadedIcon> {
...
async function* resolveLinkedIcons(
  ...
  for (const link of links ?? []) {
    ...
    if (preferredSource === "official" && isGenericOfficialIconHost(link.href)) {
      continue;
    }
...
async function* resolveCuratedIcons(skillName: string): AsyncGenerator<DownloadedIcon> {
  const candidates = [...(originalLogoCandidates[skillName] ?? []), ...(curatedIconCandidates[skillName] ?? [])];
...
function customIconSourceUrl(skillName: string, skillDetail: SkillDetails[string] | undefined): string {
  return skillDetail?.links[0]?.href ?? `https://w4w.dev/resume#${slugify(skillName)}`;
}

function createCustomIcon(skillName: string, category: string, skillDetail: SkillDetails[string] | undefined): DownloadedIcon {
  const digest = createHash("sha256").update(`${category}:${skillName}`).digest("hex");
  ...
  return {
    ...
    source: "custom",
    sourceNote: "Generated unique custom SVG because no safe unique brand, favicon, legacy, or source-traced icon was resolved.",
...
function validateManifest(manifest: IconManifest, visibleSkills: VisibleSkill[]): void {
  ...
  for (const { skillName } of visibleSkills) {
    ...
    if (!entry) {
      throw new Error(`Missing icon manifest entry for visible skill ${skillName}`);
    }
    ...
    if (existingPathSkill) {
      throw new Error(`Duplicate icon path ${entry.iconPath} for ${existingPathSkill} and ${skillName}`);
    }
    ...
    if (existingHashSkill) {
      throw new Error(`Duplicate icon content hash ${entry.sha256} for ${existingHashSkill} and ${skillName}`);
    }
...
async function buildManifest(visibleSkills: VisibleSkill[], skillDetails: SkillDetails): Promise<IconManifest> {
  ...
  for (const { category, skillName } of visibleSkills) {
    ...
    for await (const icon of resolveIconCandidates(skillName, category, skillDetails)) {
      ...
      if (duplicatePathSkill) {
        throw new Error(`Duplicate icon path ${iconPath} for ${duplicatePathSkill} and ${skillName}`);
      }
      ...
      await writeFile(path.join(tempIconOutputDir, path.basename(iconPath)), icon.bytes);
...
async function syncSkillIcons() {
  const siteVariant = siteVariantSchema.parse(await readJsonFile(siteVariantPath));
  const skillDetails = skillDetailsSchema.parse(await readJsonFile(skillDetailsPath));
  ...
  try {
    ...
    await writeFile(manifestPath, `${JSON.stringify(parsedManifest, null, 2)}\n`);
